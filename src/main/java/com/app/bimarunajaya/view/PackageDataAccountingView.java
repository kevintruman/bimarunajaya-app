...
public class PackageDataAccountingView extends ContainerLayout {

    public PackageDataAccountingView() {
        super();
        init();
    }
    ...
    private void initComponent() {
        ...
        List<CbFilter> s = Arrays.asList(new CbFilter(null, "Belum Konfirmasi"),
                new CbFilter(TypeService.approveAccounting, "Menunggu Konfirmasi"),
...
//        lyTotal.setPadding(false);
    ...
    private void initData() {
        ...
        if (r.size() == 0) {
            ...
            if (cbFilter.getValue() != null && cbFilter.getValue().getId() == null)
        ...
        grid.asSingleSelect().addValueChangeListener(e -> {
            ...
            if (e.getValue() != null) {
                ...
                if (e.getValue().getApproveStatus() == null) btConfirm.setEnabled(true);
            }
        });
    }
    ...
    private void onConfirmAll() {
        for (PackageEntity packageEntity : r) {
            packageService.approveAccounting(packageEntity.getId());
        }
        initData();
    ...
    private void onConfirm() {
        packageService.approveAccounting(grid.asSingleSelect().getValue().getId());
        initData();
        Notification.show("Berhasil diproses");
    }
